...
sudo cp "$PWD/config_files/course.subdomain.conf" "/var/lib/docker/swag/config/proxy-confs/$coursename.subdomain.conf"

# Replace <container_name> with the course name in the copied file
sudo sed -i "s/<container_name>/$coursename/g" "/var/lib/docker/swag/config/proxy-confs/$coursename.subdomain.conf"
...
echo "Please make sure that you have configured your DNS to have a CNAME record which points to ${coursename}.yourdomain.tld."
